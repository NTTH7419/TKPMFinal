...
- **Integrate** Supabase (PostgreSQL + Storage), Redis 7 via Docker (rate limiting, queue tokens, circuit breaker state, BullMQ, JWT blocklist, SSE pub/sub), Resend (email), Gemini/OpenAI API (AI summary), Mock Payment Gateway.
...
- `workshop-catalog`: Workshop CRUD (title, speaker, room, room_map_url, capacity, fee_type, price, schedule), public listing with eventual-consistent seat count via SSE endpoint + Redis Pub/Sub, workshop detail page showing AI summary, statistics dashboard for organizers (registrations, confirmed count, capacity utilization per workshop).
...
- `payment`: Mock payment gateway adapter, payment intent + webhook flow, 30-day idempotency key, circuit breaker (Closed/Open/Half-Open, threshold: 5 consecutive failures OR >50% failure rate in 30s window, gateway call timeout 5s) with graceful degradation, auto-refund when hold expires after payment succeeds, payment reconciliation worker (every 15 minutes).
...
- **New tech stack**: pnpm monorepo, NestJS, React + Vite, Supabase (PostgreSQL + Storage), Redis 7 (Docker), BullMQ, Resend, Gemini API.
- **External services required**: Supabase project (PostgreSQL + Storage), Resend API key, AI provider API key (Gemini or OpenAI). Local infrastructure: Redis 7 via Docker Compose.
