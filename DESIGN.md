...
> **Canonical token source**: `packages/ui/src/tokens/tokens.ts` — every value in this file is the source of truth for what is documented below. Edit tokens there, then run `pnpm --filter @unihub/ui build` to regenerate the CSS variable bundle and Tailwind preset. See [`packages/ui/README.md`](packages/ui/README.md) for consumer instructions.
